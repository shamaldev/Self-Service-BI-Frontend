...
return `${new Date().toISOString().split("T")[0]}_${timePeriod}`;
}, []);

const storedMessage = useMemo(() => {
try {
return localStorage.getItem(`greeting_${todayKey}_${name}_insights`);
} catch {
return null;
}
}, [todayKey, name]);
...
const newMessage = combined[Math.floor(Math.random() * combined.length)];
try {
  localStorage.setItem(`greeting_${todayKey}_${name}_insights`, newMessage);
} catch {}
return newMessage;


}, [storedMessage, todayKey, name]);
